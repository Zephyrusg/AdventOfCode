...
Function ConvertFrom-Operator{
    ...
    switch($type){
        0{
            $value = ($subValues | Measure-Object -Sum).sum
        }
        1{
            [long] $value = 1
            $subValues.ForEach{$value *= $_}
        }
        2{
            $value = ($subValues | Measure-Object -Minimum).Minimum
        }
        3{
            $value = ($subValues | Measure-Object -Maximum).Maximum
        }
        5{
            if($subValues[0] -gt $subValues[1]){
                [long] $value = 1
        ...
        6{
            [long] $value = $subValues[0] -lt $subValues[1] ? 1 : 0
        }
        7{
            [long] $value = $subValues[0] -eq $subValues[1] ? 1 : 0
        }
    }
...
Function New-AoCPackage{
    ...
    if($type -eq 4){
        ...
        $returnArray  = ConvertFrom-Operator -stringBinair $stringBinair.Substring(6) -type $type
    }
