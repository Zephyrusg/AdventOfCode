...
#$data = Get-Content -Path "$($path)\Exampleday18_2"
...
Function Find-Score{
    ...
    [int]$score = [int]::parse($finalstring)    
...
$highestnumber = 0
$snailfishnumber = 1
foreach($snailfish in $data){
    Write-Host "Starting with Snailfish:$snailfish SubCalculation: $($data.count) to go"
    $currentstring =$snailfish
    $calculations = 1
    :skipself foreach($calculation in $data){
        
        if($snailfish -eq $calculation){
            continue skipself
        }

        write-host "Snailfish: $snailfishnumber, doing SubCalculation:$calculations/$($data.count -1)"
        $currentstring = '[' + $snailfish + "," + $calculation + "]"
        ...
        $result = Find-Score -finalstring  $currentstring
        if($result -gt $highestnumber){
            $highestnumber = $result
            Write-Host "Found Higher Result: $highestnumber"
        }
    }
    $snailfishnumber++
}
